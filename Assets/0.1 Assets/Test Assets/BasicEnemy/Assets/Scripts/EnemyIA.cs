...
public class EnemyIA : MonoBehaviour
{
    ...
    PlayerMovementAi playerMethods;
    Menu menuMethods;
    ...
    void Start()
    {
        ...
        playerMethods = FindAnyObjectByType<PlayerMovementAi>();
        menuMethods = FindAnyObjectByType<Menu>();
    ...
    private void Chasing()
    {
        ...
        if (navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)    //  Control if the enemy arrive to the player location
        {
            ...
            {
                if (Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("Player").transform.position) <= 3.0)
                {
